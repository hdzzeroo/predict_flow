...
class Evaluator:
    ...
    def _load_kp_intervals(self) -> List[Tuple[float, float]]:
        ...
        csv_file_map = {
            '関越道': os.path.join(base_dir, "roadic_kannetsu.csv"),
            '東北道': os.path.join(base_dir, "roadic_touhoku.csv")
        }

        if self.road_type not in csv_file_map:
            print(f"⚠️ 未识别的道路类型 '{self.road_type}'，使用関越道")
            csv_file = csv_file_map['関越道']
        else:
            csv_file = csv_file_map[self.road_type]

        if not os.path.exists(csv_file):
            print(f"❌ 道路信息文件不存在: {csv_file}")
            return []
        ...
        for i, item in enumerate(data):
            try:
                ...
                if data_type == "predictions":
                    ...
                    # 真实数据格式：{"vertices": [...]} 或需要从其他字段构造
                    if "vertices" in item:
                        vertices = item["vertices"]
                    else:
                        # 尝试从kp_start, kp_end, time等字段构造
    ...
    def _evaluate_grid_f1(
        ...
        try:
            ...
            if not self.kp_intervals:
                ...
                return {"precision": 0.0, "recall": 0.0, "f1_score": 0.0, "grid_iou": 0.0}
    ...
    def _rasterize_polygons_kp_based(
        ...
        for i, (kp_start, kp_end) in enumerate(self.kp_intervals):
            ...
            for j in range(n_time):
                ...
                # 创建栅格矩形 (使用 shapely.geometry.box)
