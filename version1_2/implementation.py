...
def chatbot(state: TrafficState) -> dict:
    ...
    try:
        ...
        print(f"⚠️ LLM parsing failed, falling back to traditional method: {str(e)}")
        ...
        print(f"🔧 Traditional method extraction result:")
        print(f"   Route: {route}")
        print(f"   Time: {time}")
        ...
        return {
            ...
            "direction": None,                  # 传统方法无法解析方向，设为None以启用双方向模式
            "ts": time or "Not specified"
        }
    ...
    if direction_data:
        ...
        for direction, data in direction_data.items():
            ...
            try:
                ...
                print(f"  📊 Auto-adjusted parameters: eps={eps}, min_samples={min_samples}")
                ...
                # 添加回退机制：确保所有三角形都被覆盖
                clustered_indices = set(idx for cluster in dir_clusters for idx in cluster)
                unclustered = [i for i in range(len(dir_triangles)) if i not in clustered_indices]

                if unclustered:
                    print(f"  ⚠️  {len(unclustered)} triangles not clustered, adding as individual clusters")
                    for idx in unclustered:
                        dir_clusters.append([idx])
...
def report(state: TrafficState) -> dict:
    ...
    print("Generating traffic congestion hotspot location report...")
