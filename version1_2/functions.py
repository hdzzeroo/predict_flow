...
def cluster_triangles_dbscan(triangles: List[Dict[str, Any]], 
    ...
    # 改进噪声点处理：将每个噪声点作为独立聚类
    # 这样确保所有三角形都会被某个hull覆盖
    if noise_points:
        print(f"  - Found {len(noise_points)} noise points, adding as individual clusters")
        for idx in noise_points:
            cluster_list.append([idx])

    print(f"DBSCAN clustering completed:")
    print(f"  - Found {len(cluster_list)} clusters (including {len(noise_points)} single-point clusters)")
